...
namespace TellMe.Core.Types.DataServices.Remote
{
    public class RemoteStorytellersDataService : IRemoteStorytellersDataService
    {
        ...
        {
            var result = await this._apiProvider.PostAsync<FriendshipStatus>($"storytellers/{id}/add-to-friends", null).ConfigureAwait(false);
            return result;
        }

        public async Task<Result<FriendshipStatus>> RejectFriendshipRequestAsync(string id, int notificationId)
        {
            var result = await this._apiProvider.PostAsync<FriendshipStatus>($"storytellers/{id}/reject-friendship", notificationId).ConfigureAwait(false);
            return result;
        }

        public async Task<Result<FriendshipStatus>> AcceptFriendshipRequestAsync(string id, int notificationId)
        {
            var result = await this._apiProvider.PostAsync<FriendshipStatus>($"storytellers/{id}/add-to-friends", notificationId).ConfigureAwait(false);
            return result;
        }

        public async Task<Result> SendRequestToJoinAsync(string email)
        {
            var result = await this._apiProvider.PostAsync<object>($"storytellers/request-to-join", email).ConfigureAwait(false);
            return result;
        }

        public async Task<Result<StorytellerDTO>> GetByIdAsync(string storytellerId)
        {
            var result = await this._apiProvider.GetAsync<StorytellerDTO>($"storytellers/{storytellerId}").ConfigureAwait(false);
            return result;
        }
    }
}
...
namespace TellMe.iOS.Views.Badge
{
    /// <summary>
    /// A Badge view.
    /// </summary>
    public class BadgeView : UIView
    {
        ...
        public override void Draw(CGRect rect)
        {
            ...
            var numberSize = new NSString(numberString).StringSize(this.Font);

            using (CGPath badgePath = this.NewBadgePathForTextSize(numberSize))
            {
                var badgeRect = badgePath.PathBoundingBox;
                ...
                if (this.Shadow)
                {
                    ...
                    var blurSize = this.ShadowOffset;
                ...
                if (this.Shine)
                {
                    ...
                    using (CGColorSpace colorSpace = CGColorSpace.CreateDeviceRGB())
                    {
                        ...
                        using (CGGradient gradient = new CGGradient(colorSpace, shinyColorGradient, shinyLocationGradient))
                        {
                            ...
                            var shineStartY = badgeRect.Size.Height * 0.25f;
                            var shineStopY = shineStartY + (badgeRect.Size.Height * 0.4f);
